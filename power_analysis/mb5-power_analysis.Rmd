...
* *familiarization_time*: indicates how long each stimulus is exposed during familiarization (5, 10, or 20 seconds; within-infant) 
...
generate_dataset <- function(n_labs=20, n_per_lab=20, effect_sizes=list(type = .3, age = 0, "age*type"=0)) {
  ...
  # stimulus_type (ms says randomly assigned: we don't want counterbalanced per lab?)
  stimulus_type = sample(c("high","low"), length(subjID), replace=T)

  simd <- tibble(subjID, labID, procedure, test_order, stimulus_type)

  # uniform random vars
  simd$age_mos = runif(nrow(simd), min=5.0, max=12.0)
  simd$age = scale(simd$age_mos, center=T, scale=T)[,1]

  # now generate looking times for 24 trials per subject
  for(t in 1:24) {
  ...
  siml$trial_type = rep_len(c(rep("same", 12), rep("different", 12)), nrow(siml)) # each

  per_subj_trial_type = c(rep("same", 12), rep("different", 12))
