---
title: "ManyBabies 5: The Hunter & Ames Model of Infant Looking Preference"
subtitle: "Supplementary Materials: Data Simulation & Power Analysis"
output:
  pdf_document:
    toc: true
    toc_depth: 3
...
my_sim_data <- function(
    ...
    subject_c   =  0.3, # by-subject slope complexity sd
    subject_f = 0.3, # by-subject slope familiarization sd
    subject_a = 0.3, # by-subject slope age sd
    
    subject_ca = 0.3,# by-subject slope for interaction between age and complexity sd
    subject_af = 0.3, # by-subject slope for interaction between age and familiarization sd
    subject_cf = 0.3, # by-subject slope complexity*familiarization sd
    
    subject_cfa = 0.3, # by-subject slope for interaction between age, complexity and familiarization sd
...
## Variability by Lab
```{r, fig.height=17, warning=FALSE, echo=FALSE}
plot_labs <- dat_sim %>%
...
# Power Calculation with Full Data
...
run_sims <- function(filename_full, ef) {
  ...
  mod_sim <- lmer(DV ~ 1 + X_a * X_c * X_f + (1 | subj_id) + (1 | lab_id) + (1 | item_id), data=dat_sim)
...
fixed_full_plot <- sims_full_0.3 %>%
  ...
  ylab("Estimates") +
  xlab("Simulations") +
  ggtitle('Estimates of Fixed Effects for Full Data, ef = 0.3') +
  theme_bw()
...
# Power Calculation with Missing Data
...
run_sims_missing <- function(filename_missing, ef) {
  ...
  mod_sim <- lmer(DV ~ 1 + X_a * X_c * X_f + (1 | subj_id) + (1 | lab_id) + (1 | item_id), data=missing_samples)
...
filename_missing_0.5 = 'run_sims_missing_0.5.csv'
start_time <- Sys.time()
sims_missing <- purrr::map_df(1:reps, ~run_sims_missing(filename_missing = filename_missing_0.5, ef = 0.5))
end_time <- Sys.time()
end_time - start_time
```
...
```{r,  warning = FALSE, message = FALSE}
filename_missing_0.4 = 'run_sims_missing_0.4.csv'
start_time <- Sys.time()
sims_missing <- purrr::map_df(1:reps, ~run_sims_missing(filename_missing = filename_missing_0.4, ef = 0.4))
end_time <- Sys.time()
end_time - start_time
```
...
```{r,  warning = FALSE, message = FALSE}
filename_missing_0.3 = 'run_sims_missing_0.3.csv'
start_time <- Sys.time()
sims_missing <- purrr::map_df(1:reps, ~run_sims_missing(filename_missing = filename_missing_0.3, ef = 0.3))
end_time <- Sys.time()
end_time - start_time
```
...
sims_missing_0.3 <- read_csv(filename_missing_0.3, col_types = cols(
  # makes sure plots display in this order
  group = col_factor(ordered = TRUE),
  term = col_factor(ordered = TRUE)
))

fixed_missing_plot <- sims_missing_0.3 %>%
  filter(effect == "fixed") %>%
  ungroup() %>%
  arrange(term, estimate) %>%
  mutate(row = rep(seq(1:reps), 8)) %>%
  ...
  ylab("Estimates") +
  xlab("Simulations") +
  ggtitle('Estimates of Fixed Effects for Missing Data, ef = 0.3') +
  theme_bw()
...
```{r}
ran_missing_plot <- sims_missing_0.3 %>%
  filter(effect == "ran_pars") %>%
  ungroup() %>%
  arrange(group, estimate) %>%
  mutate(row = rep(seq(1:reps), 4)) %>%
  ...
  ylab("Estimates") +
  xlab("Simulations") +
  ggtitle('Estimates of Random Effects for Missing Data, ef = 0.3') +
  theme_bw()
...
```{r,  warning = FALSE, message = FALSE}
filename_missing_0.2 = 'run_sims_missing_0.2.csv'
start_time <- Sys.time()
sims_missing <- purrr::map_df(1:reps, ~run_sims_missing(filename_missing = filename_missing_0.2, ef = 0.2))
end_time <- Sys.time()
end_time - start_time
```
...
```{r,  warning = FALSE, message = FALSE}
filename_missing_0.1 = 'run_sims_missing_0.1.csv'
start_time <- Sys.time()
sims_missing <- purrr::map_df(1:reps, ~run_sims_missing(filename_missing = filename_missing_0.1, ef = 0.1))
end_time <- Sys.time()
end_time - start_time
```
...
## Summary Statistics for Power Calculation with Full Data:
...
sim_stats_full %>%
  kbl(caption = "Power for Simulations with Full Data", digits=3, align = "c") %>%
  kable_styling(full_width = T, latex_options = c("striped", "HOLD_position"))
```

## Summary Statistics for Power Calculation with Missing Data:

```{r, message = FALSE, warning = FALSE, echo = FALSE}
sim_stats_missing_0.5 <-
  read_csv(filename_missing_0.5, col_types = cols(group = col_factor(ordered = TRUE), 
                                                  term = col_factor(ordered = TRUE))) %>%
  filter(effect == "fixed") %>%
  group_by(term) %>%
  summarise(
...
sim_stats_missing_0.4 <-
  read_csv(filename_missing_0.4, col_types = cols(group = col_factor(ordered = TRUE), 
                                                  term = col_factor(ordered = TRUE))) %>%
  filter(effect == "fixed") %>%
  group_by(term) %>%
  summarise(
...
sim_stats_missing_0.3 <-
  read_csv(filename_missing_0.3, col_types = cols(group = col_factor(ordered = TRUE), 
                                                  term = col_factor(ordered = TRUE))) %>%
  filter(effect == "fixed") %>%
  group_by(term) %>%
  summarise(
...
sim_stats_missing_0.2 <-
  read_csv(filename_missing_0.2, col_types = cols(group = col_factor(ordered = TRUE), 
                                                  term = col_factor(ordered = TRUE))) %>%
  filter(effect == "fixed") %>%
  group_by(term) %>%
  summarise(
...
sim_stats_missing_0.1 <-
  read_csv(filename_missing_0.1, col_types = cols(group = col_factor(ordered = TRUE), 
                                                  term = col_factor(ordered = TRUE))) %>%
  filter(effect == "fixed") %>%
  group_by(term) %>%
  summarise(
...
sim_stats_missing <- cbind(sim_stats_missing_0.1[,1], 
                           sim_stats_missing_0.1[,4], 
                           sim_stats_missing_0.2[,4], 
                           sim_stats_missing_0.3[,4], 
                           sim_stats_missing_0.4[,4], 
                           sim_stats_missing_0.5[,4])

sim_stats_missing %>%
  kbl(caption = "Power for Simulations with Missing Data", digits=3, align = "c") %>%
  kable_styling(full_width = T, latex_options = c("striped", "HOLD_position"))
```