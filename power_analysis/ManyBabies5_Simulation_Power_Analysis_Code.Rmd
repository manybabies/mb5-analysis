...
my_sim_data <- function(
    ...
    n_simple  =  6,   # number of simple stimuli
...
plot_labs <- dat_sim %>%
  ...
  geom_vline(xintercept = 0, color = "black", linewidth = 0.3, linetype = "dotted") +
  xlab('DV') +
  ylab('Lab') +
  xlim(c(-2, 2)) +
...
my_sim_data_variance <- function(
  ...
  temp <- temp %>%
    mutate(trial_i_var = ifelse(trial_num > 0, rnorm(nrow(temp), 0, trial_num*(increase_sd / 12)), 0))
...
filename_heteroskedasticity = 'filename_heteroskedasticity.csv'
start_time <- Sys.time()
...
missing_samples_trial <- dat_sim %>%
  mutate(nas = rbinom(n(), 1, 0.95 - ifelse(trial_num > 0, trial_num*0.05, 0))) %>%
  mutate(DV = ifelse(nas == 1, DV, NA))
